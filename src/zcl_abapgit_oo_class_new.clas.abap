class zcl_abapgit_oo_class_new definition
  public
...
*>>> MBT
*   lt_methods is sorted alphabetically which does not match how
*   methods are ordered in SE24. Therefore, we get get the source
*   code location for each method and sort the implementations
*   the same way as the source
    TYPES:
      BEGIN OF ty_method_source_interval,
        name     TYPE seocpdname,
        interval TYPE cl_oo_source_scanner_class=>type_source_interval,
      END OF ty_method_source_interval .
    TYPES:
      ty_method_source_interval_tt TYPE TABLE OF ty_method_source_interval.

    DATA: ls_method_impl TYPE ty_method_source_interval,
          lt_method_impl TYPE ty_method_source_interval_tt.

    LOOP AT lt_methods INTO lv_method.
      ls_method_impl-name = lv_method.
      ls_method_impl-interval = lo_scanner->get_method_impl_interval( lv_method ).
      INSERT ls_method_impl INTO TABLE lt_method_impl.
    ENDLOOP.
    SORT lt_method_impl BY interval-begin-line.

*  LOOP AT lt_methods INTO lv_method.
   LOOP AT lt_method_impl INTO ls_method_impl.
      lv_method = ls_method_impl-name.
**<<< MBT
      TRY.
